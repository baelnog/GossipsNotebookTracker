...
namespace ChecklistTracker.View
{
    ...
    /// Fork of https://github.com/CommunityToolkit/Windows/tree/main/components/Collections/src/AdvancedCollectionView/AdvancedCollectionView.cs
    ...
    public partial class AdvancedCollectionView : IAdvancedCollectionView, INotifyPropertyChanged, ISupportIncrementalLoading, IComparer<object>
    {
        ...
        */
        ...
        private void AttachPropertyChangedHandler(IBindableVector? items)
        {
            ...
            {
                if (items.GetAt(i) is INotifyPropertyChanged propChanged)
                {
        ...
        private void DetachPropertyChangedHandler(IBindableVector? items)
        {
            if (!_liveShapingEnabled || items == null)
            {
                return;
        ...
        private void HandleFilterChanged()
        {
            ...
            var viewIndex = 0;
            for (var index = 0; index < _source.Count && viewIndex <= _view.Count; index++)
            {
            ...
            else if (_filter != null)
            {
                ...
                else
                {
                    for (int i = 0, j = 0; i < _source.Count; i++)
                    {
                        ...
                        if (j < _view.Count && _view[j] == _source[i])
                        {
                            j++;
                        }
                    }
